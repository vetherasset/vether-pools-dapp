...
export const getSwapOutput = (inputAmount, pool, toVether) => {
  // formula: (x * X * Y) / (x + X) ^ 2
  const x = bn(inputAmount)
  const X = toVether ? bn(pool.asset) : bn(pool.vether) // input is token if toVether
  const Y = toVether ? bn(pool.vether) : bn(pool.asset) // output is vether if toVether
...
export const getSwapInput = (toVether, pool, outputAmount) => {
...
export const getSwapSlip = (inputAmount, pool, toVether) => {
  // formula: (x) / (x + X)
  const x = bn(inputAmount)
  const X = toVether ? bn(pool.asset) : bn(pool.vether) // input is token if toVether
...
export const getSwapFee = (inputAmount, pool, toVether) => {
  // formula: (x * x * Y) / (x + X) ^ 2
  const x = bn(inputAmount)
  const X = toVether ? bn(bn(pool.asset)) : bn(pool.vether) // input is token if toVether
  const Y = toVether ? bn(pool.vether) : bn(bn(pool.asset)) // output is vether if toVether
...
export const getDoubleSwapFee = (inputAmount, pool1, pool2) => {
  ...
  const fee1Token = getValueOfVetherInToken(fee1, pool2)
  const result = fee2.plus(fee1Token)
  return result
}

export const getValueOfTokenInVether = (input, pool) => {
  // formula: ((a * V) / A) => V per A (Vetherper$)
  const a = bn(input)
  const V = bn(pool.vether)
...
export const getValueOfVetherInToken = (input, pool) => {
  // formula: ((v * A) / V) => A per V ($perVether)
  const v = bn(input)
  const V = bn(pool.vether)
  const A = bn(bn(pool.asset))
...
export const getValueOfToken1InToken2 = (input, pool1, pool2) => {
  ...
  const VperT1 = getValueOfTokenInVether(input, pool1)
  const T2perV = getValueOfVetherInToken(one, pool2)
...
export const getStakeUnits = (stake, pool) => {
  ...
  const v = bn(stake.vether)
  const t = bn(stake.token)
  const V = bn(pool.vether).plus(v) // Must add r first
...
export const getPoolShare = (unitData, pool) => {
  // formula: (vether * part) / total; (token * part) / total
  ...
  const vether = V.times(units).div(total)
  const stakeData = {
    token: token,
    vether: vether,
  }
  // console.log((stakeData.token).toFixed(0), (stakeData.vether).toFixed(0))
  return stakeData
}
