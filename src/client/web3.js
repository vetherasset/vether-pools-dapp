...
import POOLS from '../artifacts/VetherPools2.json'

export const ETH = '0x0000000000000000000000000000000000000000'
//export const VETH = '0x95d0c08e59bbc354ee2218da9f82a04d7cdb6fdf'
//export const POOLS_ADDR = '0xCFE254e64Bb766bDb0998801F7b9F2E6762a92DB'
export const VETH = '0x4ba6ddd7b89ed838fed25d208d4f644106e34279'
...
export const getPoolsData = async (poolArray) => {
    ...
    for (let i = 0; i < poolArray.length; i++) {
        ...
        let poolData = {
            ...
            'name': name,
            'address': poolArray[i],
            'price': +poolDataRaw.vether / +poolDataRaw.asset,
            'volume': +poolDataRaw.volume,
            'vether': +poolDataRaw.vether,
            'asset': +poolDataRaw.asset,
            'depth': 2 * +poolDataRaw.vether,
...
export const getWalletData = async (poolArray) => {
    ...
    tokens.push({
        'symbol': 'VETH',
        'name': 'Vether',
        'balance': await getTokenContract(VETH).methods.balanceOf(address).call(),
        'address': VETH
    })
...
export const filterWalletByPools = async (poolsData, walletData) => {
    ...
    const tokens = wallet.filter((item) => pools.includes(item) || item === VETH)
    return tokens
}
...
export const getStakesData = async (address, poolArray) => {
    ...
    for (let i = 0; i < poolArray.length; i++) {
        ...
        let vether = await contractPool.methods.getStakerShareVether(address, poolArray[i]).call()
        ...
        let stake = {
            ...
            'vether': vether,
            'asset': asset,
            'vetherStaked': +stakeDataRaw.asset,
